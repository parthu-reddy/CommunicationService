...
public class ChatImageUploadController {
    ...
    public ResponseEntity<Map<String, Object>> uploadImage(
        ...
        if (currentImageCount >= 4) {
            ...
            return ResponseEntity.badRequest().body(Map.of(
                    "success", false,
                    "message", "Maximum limit of 4 images per user reached for this session"
            ));
        }
        ...
        try {
            ...
            ChatMessageDto savedMessage = messageService.saveMessage(
                    sessionId,
                    userId,
                    publicUrl,   // content is the image URL
                    "IMAGE"
            );
            ...
            if (readers.hasNext()) {
                ...
                try {
                    ...
                    if (width > MAX_IMAGE_DIMENSION || height > MAX_IMAGE_DIMENSION) {
                        throw new Exception("Image dimensions (" + width + "x" + height
                                + ") exceed the " + MAX_IMAGE_DIMENSION + "x" + MAX_IMAGE_DIMENSION + " safety limit.");
                    }
                } finally {
                    reader.dispose();
                }
